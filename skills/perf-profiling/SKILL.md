---
name: perf-profiling
description: "Linux perf profiling for low-latency systems. Load when: flamegraph, perf stat, cache misses, TLB, NUMA, eBPF, jitter, heap profiling, off-CPU analysis, hardware counters, IPC, latency spikes."
license: MIT
user-invocable: true
metadata:
  author: vanillagreen
...
Documentation lookup order: local skill files, then ctx7 CLI, then web fallback.
